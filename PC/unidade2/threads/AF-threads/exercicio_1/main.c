...
void* incrementador_global(void* arg) {
    int numero_loops = *((int*)arg);

    for (int i = 0; i < numero_loops; i++) {
        contador_global++;
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < n_threads; i++) {
        pthread_create(&threads[i], NULL, incrementador_global, (void*)&n_loops);
    }

    // esperar threads terminarem execução
    for (int i = 0; i < n_threads; i++) {
