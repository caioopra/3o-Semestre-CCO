...
// paralelizando a criação da matriz c
        #pragma omp parallel for firstprivate(i) schedule(dynamic, 1)  // mantem o valor de i
        for (j = 0; j < cols_right; ++j) {
            out[i * cols_right + j] = 0;  // inicialmente com valor 0
...
int main(int argc, char* argv[]) {
    ...
    // quantidade de threads disponiveis para uso
    omp_set_num_threads(omp_get_num_procs());  // feito por padrao
